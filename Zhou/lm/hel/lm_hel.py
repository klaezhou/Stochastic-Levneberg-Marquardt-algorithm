...
class PINN_LM:
    ...
    def LM(self,opt_num, step,modify=True,bd_tol=0):
        ...
        def fx_fun(params)->np.array:
            ...
            f_inside=f_inside.view(-1)
        
            fx = du_dyy+du_dxx + ((2*pi)**2)*torch.sin(2*pi*self.X_inside[:,0])*torch.sin(2*pi*self.X_inside[:,1])+\
                ((2*pi)**2)*f_inside
        ...
        def J_func(params)->torch.tensor:
            ...
            def Inter(params, input):
                ...
                def floss(func_params,input):      
                    u=fm(input,func_params)
                    d2u = jacrev(jacrev(fm))(input,func_params)
                    du_dyy=d2u[1][1] 
                    du_dxx=d2u[0][0]     
                    
                    fx = du_dyy+du_dxx + ((2*pi)**2)*torch.sin(2*pi*input[0])*torch.sin(2*pi*input[1])+((2*pi)**2)*u
                    # print('du_dY',du_dY)
                    return fx
            ...
            while (k < kmax):        
                ...
                if  F_pnew<1e-9:  # 满足收敛条件
                    ...
                    if ratio > yi and torch.norm(gkF,p=2)**2>yi2/mu:
                        ...
                        if k%1==0:
                            print(f"steps {k} accept move | Loss = {F_p.item()}, Loss new = {F_pnew.item()} | parameter #: {opt_num} | Elapsed: {elapsed_time_ms:.1f}ms| rug: {mu}| gk:{(torch.norm(gk,p=2)**4).item()}") 
    ...
    def adam_step(self, itr, state=None, step_size=0.001, b1=0.9, b2=0.999, eps=1e-8):
            ...
            def fx_fun(params)->np.array:
                ...
                f_inside=f_inside.view(-1)
                fx = du_dyy+du_dxx + ((2*pi)**2)* torch.sin(2*pi*self.X_inside[:,0])*torch.sin(2*pi*self.X_inside[:,1])+((2*pi)**2)*f_inside
    ...
    def lbfgs_step(self, max_iter=100, history_size=100, gtol=1e-10):
            ...
            def fx_fun(params):
                ...
                f_inside=f_inside.view(-1)
                fx_in= du_dyy+du_dxx + ((2*pi)**2)* torch.sin(2*pi*self.X_inside[:,0])*torch.sin(2*pi*self.X_inside[:,1])+((2*pi)**2)*f_inside
                f_bd = f(params, self.X_boundary)
            ...
            def J_func(params):
                ...
                def Inter(params, input):
                    ...
                    def floss(func_params, input):
                        ...
                        return      du_dyy+du_dxx + ((2*pi)**2)*torch.sin(2*pi*input[0])*torch.sin(2*pi*input[1])+((2*pi)**2)*u
